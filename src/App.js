...
function App() {
  ...
  const fetchTasks = async () => {
    const res = await fetch('https://taskmanager7.netlify.app/tasks')
    ...
    const fetchTask = async (id) => {
      const res = await fetch(`https://taskmanager7.netlify.app/tasks/${id}`)
      const data = await res.json()
  ...
  const deleteTask = async (id) => {
    await fetch(`https://taskmanager7.netlify.app/tasks/${id}`, {
      method: 'DELETE'
    })
